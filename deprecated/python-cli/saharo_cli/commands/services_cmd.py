...
def _set_desired_custom_services(
    client,
    *,
    server_id: int,
    ...
    # Backward compatibility for older saharo_client builds without `services` kwarg.
    if services is not None:
        try:
            return client.admin_server_desired_custom_services_set(
                server_id,
                services=services,
                **kwargs,
            )
        except TypeError as exc:
            if "unexpected keyword argument 'services'" not in str(exc):
                raise
    return client.admin_server_desired_custom_services_set(server_id, **kwargs)
...
def desired_set(
    ...
    try:
        ...
        if not codes:
            ...
            raise typer.Exit(code=2)
        out = _set_desired_custom_services(
            client,
            server_id=server_id,
            service_codes=codes,
