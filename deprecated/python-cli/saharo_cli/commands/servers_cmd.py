...
from saharo_client.resolve import ResolveError, find_server_by_name, resolve_agent_id_for_agents, resolve_server_id_for_servers
...
def _resolve_agent_id_after_timeout(
        *,
        cfg,
        base_url_override: str | None,
        agent_name: str,
    ...
    while True:
        client = make_client(cfg, profile=None, base_url_override=base_url_override)
        try:
            return int(resolve_agent_id_for_agents(client, agent_name))
        except ApiError as exc:
...
def _resolve_registration_with_fallback(
        *,
    ...
    if reg_result.registered and reg_result.agent_id:
        return int(reg_result.agent_id)
    fallback_agent_id = _resolve_agent_id_after_timeout(
        cfg=cfg,
    ...
    if fallback_agent_id is not None:
        if not json_out:
            console.warn(
                "Agent state file is not populated yet; using API-discovered registration."
            )
        return fallback_agent_id
    return _require_registration(reg_result, timeout_s=timeout_s)
...
def bootstrap(
    ...
    if local:
        ...
        if agent_id is None:
            ...
            agent_id = _resolve_registration_with_fallback(
