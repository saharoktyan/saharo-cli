...
def test_render_env_includes_root_admin_secret(tmp_path) -> None:
    ...
    assert "NODE_PROVISIONER_URL=http://node-provisioner:8090" in env_content
    assert "NODE_PROVISIONER_AUTH_TOKEN=jwt-secret" in env_content


def test_render_compose_includes_node_provisioner_service(tmp_path) -> None:
    inputs = BootstrapInputs(
        api_url="https://example.com",
        api_url_original=None,
        host_name="Host API",
    ...
    compose_content = render_compose(inputs)
    assert "node-provisioner" in compose_content
    assert "saharo_node_provisioner" in compose_content
    assert "registry.example.com/saharo/v1/node-provisioner:1.2.3" in compose_content
