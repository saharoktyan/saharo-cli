...
def render_compose(inputs: BootstrapInputs) -> str:
    ...
    node_provisioner_image = f"{inputs.registry}/saharo/v1/node-provisioner:{inputs.tag}"
    ...
    try:
        ...
        lines = [
                ...
                "      retries: 5",
                "  node-provisioner:",
                f"    image: {node_provisioner_image}",
                "    container_name: saharo_node_provisioner",
                "    restart: unless-stopped",
                "    env_file:",
                "      - ./.env",
...
def render_env(inputs: BootstrapInputs, *, include_root_secret: bool) -> str:
    ...
    internal_token = inputs.jwt_secret
    lines = [
        ...
        "NODE_PROVISIONER_URL=http://node-provisioner:8090",
        "NODE_PROVISIONER_TIMEOUT_S=20",
        "NODE_PROVISIONER_AUTH_HEADER=X-Internal-Token",
        f"NODE_PROVISIONER_AUTH_TOKEN={internal_token}",
        "NODE_PROVISIONER_BIND=0.0.0.0:8090",
        "NODE_PROVISIONER_INTERNAL_AUTH_HEADER=X-Internal-Token",
        f"NODE_PROVISIONER_INTERNAL_AUTH_TOKEN={internal_token}",
